...
PRIMARY_RUNTIME_ARTIFACT_ID = "llm_outline_compare_runtime"
VISIBLE_CAMPAIGNS = tuple(campaign for campaign in LLM_CAMPAIGNS if campaign.runtime_visible)
...
def _load_year_refs(input_file: str) -> tuple[str, str]:
    ...
    return (prev if isinstance(prev, str) else "", curr if isinstance(curr, str) else "")
...
def _validate_outline_payload(
    ...
    *,
    ...
    if payload.get("artifact_id") != expected_artifact_id:
        reasons.append("artifact_id mismatch")
    ...
    if provenance is None:
        ...
        input_file_raw = provenance.get("input_file")
        if isinstance(input_file_raw, str):
            input_file = input_file_raw

    return (len(reasons) == 0, reasons, run_label, input_file)


def _validate_outline_research_payload(payload: dict[str, Any], lens: str) -> bool:
...
def build_variant_rows(
    ...
    *,
    ...
    stats = {"targets": 0, "present": 0, "valid": 0, "invalid": 0, "missing": 0}
    started = time.monotonic()
    last_heartbeat = started
    total_cases = len(cases)

    for processed_cases, case_any in enumerate(cases, start=1):
        now = time.monotonic()
        if verbose_progress or now - last_heartbeat >= progress_interval_sec:
            elapsed = int(now - started)
            print(
                "[progress] variants_index "
                + f"cases={processed_cases}/{total_cases} rows={len(rows)} "
                + f"targets={stats['targets']} present={stats['present']} missing={stats['missing']} "
                + f"elapsed={elapsed}s",
                flush=True,
            )
            last_heartbeat = now
        ...
        for campaign in VISIBLE_CAMPAIGNS:
            ...
            for lens in campaign_lenses(campaign.input_mode):
                rel = canonical_outline_compare_relative_path(
                    ticker=ticker,
                    section=section,
                    year_from=year_from,
                    year_to=year_to,
                ...
                if present:
                    stats["present"] += 1
                    try:
                        output_payload_raw = read_json(abs_path)
                        output_payload = as_dict(output_payload_raw)
                        if output_payload is None:
                            reasons = ["output root is not object"]
                        else:
                            valid, reasons, run_label, input_file = _validate_outline_payload(
                                output_payload,
                                lens=lens,
                                campaign_provider=campaign.model_provider,
                                campaign_model=campaign.model_name,
                                expected_artifact_id=PRIMARY_RUNTIME_ARTIFACT_ID,
                            )
                            if input_file:
                ...
                if outline_compare_insight_present:
                    try:
                        insight_payload_raw = read_json(outline_compare_insight_abs_path)
                        insight_payload = as_dict(insight_payload_raw)
                        if insight_payload is not None:
                            outline_compare_insight_valid, _reasons, _label, _input = _validate_outline_payload(
                                insight_payload,
                                lens=lens,
                                campaign_provider=campaign.model_provider,
                                campaign_model=campaign.model_name,
                ...
                rows.append(
                    {
                        ...
                        "lens": lens,
                        "source_id": "edgar",
                        "detector_id": PRIMARY_RUNTIME_ARTIFACT_ID,
                        ...
                        "outline_compare_present": present,
                        "outline_compare_valid": valid,
                        "outline_compare_expected_repo_path": repo_path,
                        "outline_compare_request_url": request_url,
...
def build_payload(rows: list[dict[str, Any]]) -> dict[str, Any]:
    return {
        ...
        "provenance": {
            ...
            "notes": [
                "Built from canonical runtime outline-compare expectations in scripts/lab_output_tracks.py",
                "Public variants index covers runtime-visible campaigns only.",
            ],
        },
    }
...
def build_report_lines(
    ...
    lines.append("- primary_variant_artifact: `llm_outline_compare_runtime`")
    ...
    failures = [row for row in rows if not row.get("valid")]
    if not failures:
        lines.append("- none")
        return lines
    for row in failures:
        lines.append(
            "- "
            + f"{row['campaign_id']} {row['ticker']} {row['year_from']}-{row['year_to']} {row['lens']}: "
...
def main(argv: Optional[list[str]] = None) -> int:
    ...
    write_json(out_path, build_payload(rows))
    write_text(report_path, build_report_lines(registry_path, out_path, rows, stats))
