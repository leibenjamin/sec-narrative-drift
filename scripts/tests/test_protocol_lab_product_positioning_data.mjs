...
test("visible story copy stays aligned to the softened public wording", () => {
  ...
  assert.match(homeSource, /Browse the 3 fixtures/)
  assert.match(homeSource, /Answer first/)
  assert.match(companiesSource, /Document Protocol Lab/)
  assert.match(companiesSource, /Open the NVDA fixture/)
  assert.match(companiesSource, /Current pilot fixtures/)
  assert.match(companySource, /Filing answer first/)
  assert.match(companySource, /Boundary note/)
  assert.match(companySource, /Available reads/)
  assert.match(companySource, /Read order/)
  assert.match(companySource, /Back to 3 fixtures/)
  assert.match(companySource, /Primary read/)
  assert.match(companySource, /Comparison read/)
  assert.match(useCaseGuideSource, /objective: "Fastest strong signal"/)
  assert.match(useCaseGuideSource, /ticker: "NVDA"/)
  assert.match(useCaseGuideSource, /objective: "Policy-heavy contrast"/)
  assert.match(useCaseGuideSource, /ticker: "LLY"/)
  assert.match(useCaseGuideSource, /objective: "Restraint \/ skeptic check"/)
  assert.match(useCaseGuideSource, /ticker: "KO"/)
  assert.match(labPanelSource, /Case comparison/)
  assert.match(labPanelSource, /Protocol meaning/)
  assert.match(labPanelSource, /Audit route/)
  assert.match(labPanelSource, /Primary read \/ Comparison read \/ Secondary comparison \/ Control read/)
