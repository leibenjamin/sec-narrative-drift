...
export default function ExecutiveSummary({
  ...
  return (
    <section className="rounded-xl border border-white/10 bg-slate-900/60 p-5 shadow-[0_18px_40px_rgba(2,6,23,0.45)]">
      <div className="flex flex-wrap items-start justify-between gap-4">
        ...
        <button
          type="button"
          className="inline-flex items-center rounded-md border border-sky-400/40 bg-sky-400/10 px-3 py-2 text-xs font-medium text-sky-100 hover:bg-sky-400/20 disabled:cursor-not-allowed disabled:opacity-60"
          onClick={() => {
          ...
          {topRisers.length ? (
            <div className="mt-2 flex flex-wrap gap-2 text-xs">
              {topRisers.map((term) => (
                <span
                  key={`riser-${term}`}
                  className="rounded-full border border-emerald-400/40 bg-emerald-400/15 px-2 py-1 text-emerald-100"
                >
                  {term}
                </span>
              ))}
          ...
          {topFallers.length ? (
            <div className="mt-2 flex flex-wrap gap-2 text-xs">
              {topFallers.map((term) => (
                <span
                  key={`faller-${term}`}
                  className="rounded-full border border-rose-400/40 bg-rose-400/15 px-2 py-1 text-rose-100"
                >
                  {term}
                </span>
              ))}
