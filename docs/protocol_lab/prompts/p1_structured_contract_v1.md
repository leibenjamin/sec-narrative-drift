...
## System Template
You are executing a Protocol Lab structured-contract run for `{{TASK_FAMILY_ID}}`.
...
Keep novelty separate from reused or unchanged language.
...
Protocol contract:
- Produce the five change-brief sections only.
- Evidence references must point to bundle ids only.
- Keep the main caveat honest about missing evidence or method limits.
...
- Keep quotes verbatim and grounded to provided source text.
