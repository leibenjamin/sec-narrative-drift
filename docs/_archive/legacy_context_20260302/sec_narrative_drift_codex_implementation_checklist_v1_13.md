# SEC Narrative Drift - Codex Implementation Checklist (v1.13)
**Use with:** `docs/sec_narrative_drift_codex_spec_v1_13.md` (canonical spec)  
**Purpose:** Turn the spec into small, unambiguous Codex tasks that each end with a runnable, verifiable artifact.

Status (2026-01-05): Tickets 0-16 are implemented in the repo. Use this checklist for reference/regression; active backlog lives in `docs/sec_narrative_drift_postlaunch_roadmap_and_ticket_pack_v1_2.md`.

**Adjustments (2026-01-28):**
- Sample fixtures are **opt-in only** (no automatic fixture usage in pipelines).
...
**Goal:** Yearxyear heatmap with click selection.
...
- Handles small matrices (3x3) and larger (10x10)
...
**Goal:** Refresh three featured companies (examples; actual list is `scripts/universe_featured.json`).
...
## Prompt template
**Task:** Implement Ticket X from `docs/sec_narrative_drift_codex_implementation_checklist_v1_13.md`.  
**Constraints:** Follow the canonical spec `docs/sec_narrative_drift_codex_spec_v1_13.md`. Don't change JSON field names.  
...
# Postlaunch tickets (v1.2)

You have already completed Tickets 0-16. Postlaunch backlog lives in:
- `docs/sec_narrative_drift_postlaunch_roadmap_and_ticket_pack_v1_2.md` (roadmap + new tickets; many are already applied)

Reference-only (superseded by applied patch pack):
- `docs/sec_narrative_drift_accuracy_signal_upgrade_v1_1.md`

## Prompt template for postlaunch work
**Task:** Implement Ticket <ID> from `docs/sec_narrative_drift_postlaunch_roadmap_and_ticket_pack_v1_2.md`.

**Constraints:**
- Keep the site static/precomputed (no user accounts, no server DB).
- Never ship full filing text; excerpts only.
- Preserve UX: fast initial load, lazy-load per-company JSON.

**Definition of done:**
- Meets the ticket's acceptance criteria.
- `npm run build` passes.
- If scripts changed: regenerate one demo ticker locally and confirm JSON schema still matches spec v1.13.
