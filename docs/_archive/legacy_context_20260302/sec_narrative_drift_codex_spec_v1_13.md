# SEC Narrative Drift -- Codex-Ready Spec (v1.13)
...
Status (2026-01-05): Canonical for product + data contract. Extraction algorithm details are maintained in `docs/sec_risk_extraction_upgrade_codex_pack_v2.md` and supersede the extraction section below; public JSON schemas remain unchanged.

**Adjustments (2026-01-28):**
- Sample fixtures are **opt-in only** (no automatic fixture usage).
- There is **no special pipeline** for AAPL/NVDA/TSLA; all tickers use the same build/refresh flow.
- Featured tickers are configured via `scripts/universe_featured.json`.

## Canonical docs
Treat `docs/00_README_doc_index.md` as the **only canonical index**. If anything conflicts, patch the docs instead of improvising.

Primary build docs for Codex:
- `docs/sec_narrative_drift_codex_spec_v1_13.md` (this file)
...
- **Companies:** precomputed **3 featured** tickers by default (recommended: `AAPL`, `NVDA`, `TSLA`).
  *Note:* featured list is configurable via `scripts/universe_featured.json`; there is no special pipeline for those tickers.
- **Visuals:** (1) drift timeline, (2) yearxyear similarity heatmap, (3) term shift bars, (4) compare pane with highlighted excerpts
...
- `User-Agent: Ben Lei <contact@benlei.org>`
...
**Row 2:** Similarity heatmap (yearxyear) -- click cell to compare those two years  
...
Construct URL using the formula in Section2.1C.
...
Note: The current extractor uses the BlockDoc/TOC-scoring flow documented in `docs/sec_risk_extraction_upgrade_codex_pack_v1.md`. This section remains as a high-level outline only.
...
### 7.2 Yearxyear similarity matrix
...
**Note:** `cosineSimilarity` is an NxN square matrix aligned to `years` (same order).
```json
{
  "section":"10k_item1a",
...
**Notes**
- This file powers the **Key language shifts** panel.
- Terms may be **unigrams or phrases** (e.g., bigrams). Phrases are space-joined.
- Primary lists are log-odds PMI phrases; optional alternate lists (TextRank) appear as `topRisersAlt` / `topFallersAlt`.
- Term entries are objects with required `term`/`score` plus optional stats (`z`, counts, per-10k deltas, `distinctive`) and optional `includes`.
  ...
  "yearPairs": [
    {
      ...
      "topRisers": [
        {
          ...
          "deltaPer10k": 18.4,
          "distinctive": true,
          "includes": ["supply chain", "supply-chain"]
        },
        {
          "term": "pandemic",
          "score": 4.8,
      ...
      "summary": "2020 adds health + supply-chain phrasing; older product-line risks fade.",
      "topRisersAlt": [
        { "term": "supply chain disruption", "score": 0.031 }
      ],
      "topFallersAlt": [
        { "term": "legacy music devices", "score": -0.019 }
      ],
      "summaryAlt": "TextRank highlights phrases that become more central in 2020 vs 2019."
    }
  ]
}
```
...
## 10) Compare pane highlight rules (updated, still deterministic)
When user selects a year-pair:
1) Choose a `method` (default: `log_odds_phrase_pmi`).
2) Highlight the union of top **15 risers + 15 fallers** for the chosen method.
   - Highlight matching is word-boundary aware and phrase-aware.
   - Safety filters apply: drop ultra-common tokens, honorifics (e.g., "ms"), and 1-2 char fragments.
3) Pick "representative paragraphs" by score:
   - `paragraph_score = count(riser_terms) - count(faller_terms)`
   - (optional upgrade) add IDF weighting so boilerplate paragraphs don't dominate.
4) Show top 3 paragraphs from each year, with highlighted terms.

This remains deterministic and auditable.
...
OK pipeline scripts + fixtures + strict contracts.  
**Add:** unit tests for extraction patterns on fixtures.

---

# Appendix C -- Release checklist (MVP)
- [ ] UI works with precomputed featured JSON (examples: AAPL/NVDA/TSLA)
